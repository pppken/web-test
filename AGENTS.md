...
`configure({ mode, binarize, threshold, smooth, shear, debug, storageKey, onChange })`。
...
5. **横幅を `PRE_OUT_WIDTH` = 600px に合わせて伸び縮みさせる**（`resample()`）。
   伸ばすときは線形補間、縮めるときは出力の 1px が覆う区間の面積平均。
...
**出力の横幅は以前は「常に 2 倍」（1280px 幅の枠なら 2640px）だったが、幅を抑えるため
600px 固定にした。** 枠が広いと縮めることになり、そのぶん最細バーが細る（860px 幅の枠なら
約 0.7 倍）。検出画像ダイアログの「最細バー」の `出力 Npx` が 2px を切るようなら
`PRE_OUT_WIDTH` を広げること。合成画像での測定（下の表）は 2 倍にしていた頃のもの。
...
#### 集約後の二値化（binarize。検証中）

`configure({ binarize: true })` で、集約して出力の幅（`PRE_OUT_WIDTH`）に合わせた**あとに**
黒白へ割る（しきい値は `binarizeCurve()`、割るのは `buildImage()`。既定は false で、
このページは `app.js` の `setupPreprocess()` で true を渡している）。true のときは
`threshold` の指定より優先する（どちらも集約後の二値化なので、同時には掛けない）。
集約は「前処理」ボタンの選択（中央値など）のまま。検出画像ダイアログには `中央値 → 二値化` と出る。

- しきい値は x ごとに前後 `PRE_BIN_WINDOW`（48px、集約後・伸び縮み前の尺）の窓の最小と最大の
  中点。窓の中の明暗差が `PRE_BIN_MIN_RATIO`（波形全体の 25%）に満たない場所は白とみなす
  （しきい値を -1 にする）。曲線は波形と一緒に `resample()` で出力の幅へ合わせてから割る。
- **エッジの位置は出力の 1px 単位に丸められる。** 灰色のまま渡せば ZXing-C++ 側の鋭化と
  二値化がサブピクセルのエッジ位置を使えるが（「こちら側で二値化しない」の項）、割ると
  それを捨てる。600px に縮めているいまは、出力の 1px が入力の 1px より粗いこともある。
- **以前は集約の前に段ごとに割っていた**（`binarizeRows()`。割ったあとは平均で集約）。
  集約のあとに割るよう変えた。
- 入れた理由は**ラベルの縁の灰色**。灰色のまま渡すと、ラベルの外の台紙や背景が中途半端な
  暗さで残り、ZXing-C++ の行ごとのヒストグラムが灰色ごと黒に倒してクワイエットゾーンが
  消える、という見立て。ヘッドレスの合成画像（CODE39・module 4px・ぼかし 1px・
  ラベルの余白 40px、ZXing-C++、`'ab'`）での実測は次のとおり。
  ...
  **0% になったのは、出力の左右に白の余白（`PRE_PAD_X` = 40）を足していた頃だけ**で、
  ...
  確かめてはいない）。余白を戻すなら二値化もセットで入れること。

- 二値化しても、ラベルの縁のすぐ外（窓の半分、約 24px）は黒く残る（窓に白いラベルと灰色が
  同時に入るため）。ラベルの余白がそれより狭いとクワイエットゾーンにかかりうる。
- 荒れた印字（mean / median を比べた表と同じ、module 3px・傾き 1.2 度・縁のゆらぎ ±1px）での
  検出率はまだ測っていない。

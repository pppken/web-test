...
- **`tryRotate` が効くので 90 度回転は渡さない**（`needsRotation()` が false）。
  左右の帯（`SCAN_PAD_X`。塗り方は `SCAN_PAD_FILL`）は ZXing / Quagga2 と同じく足す。
...
- 同梱ライブラリの経路（ZXing / ZXing-C++ / Quagga2）では、切り出した画像の**左右に幅 `SCAN_PAD_X`
  （いまは 40px。以前は一時的に 0 にしていた）の帯**を足してから渡す。枠いっぱいにバーコードが
  写っているとクワイエットゾーンが足りず読めないため。回転経路でもバーが並ぶのは canvas の
  横方向なので、足す位置は正立時と同じ。`BarcodeDetector` には足さない（端末側の実装に任せる）。
  - **帯の塗り方は `SCAN_PAD_FILL` で選ぶ（試験的実装）。いまは `'edge'`**：切り出した画像の
    左端・右端の 1 列を帯の幅いっぱいに引き伸ばして塗る（行ごとに端の画素の色が続く）。
    `'white'` にすると従来どおり白で塗る。
  - `'edge'` の狙いは、枠の端に台紙や背景の灰色が写っているときに、真っ白な帯との境目に
    段差（偽のバー）を作らないこと（前処理で白い余白を足したら、背景が暗い灰色のときだけ
    読めなくなった件。barcode-preprocess.js の「試して外したもの」を参照）。
  - 端がバーの途中で切れていると、その黒が帯の幅いっぱいに伸びる。**`'edge'` は
    クワイエットゾーンを補う働きをしない**ので、枠いっぱいに写したときは `'white'` のほうが読める。

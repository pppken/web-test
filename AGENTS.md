...
#### 集約前の二値化（binarize。検証中）

`configure({ binarize: true })` で、集約の**前に**段ごとに黒白へ割る（`binarizeRows()`。
既定は false で、このページは `app.js` の `setupPreprocess()` で true を渡している）。
しきい値は x ごとに前後 `PRE_BIN_WINDOW`（48px）の窓の最小と最大の中点で、窓の中の明暗差が
その段の明暗差の `PRE_BIN_MIN_RATIO`（25%）に満たない場所は白とみなす。傾きは割る前の灰色で測る。

- **割ったあとの集約は常に平均**（「前処理」ボタンの選択に依らない）。0/255 の中央値は多数決に
  なってエッジが 1px 単位に丸められるが、平均なら「何割の段が黒か」になり、サブピクセルの
  エッジ位置が残る。検出画像ダイアログには `二値化 → 平均` と出る。
- 入れた理由は**ラベルの縁の灰色**。灰色のまま集約して `normalize()` で伸ばすと、ラベルの外の
  台紙や背景が中途半端な暗さで残り、ZXing-C++ の行ごとのヒストグラムが灰色ごと黒に倒して
  クワイエットゾーンが消える、という見立て。ヘッドレスの合成画像（CODE39・module 4px・
  ぼかし 1px・ラベルの余白 40px、ZXing-C++、`'ab'`）での実測は次のとおり。

  | 背景（ラベルは 225） | 二値化なし（median） | 二値化あり |
  | --- | --- | --- |
  | 225（背景なし） | 100% | 100% |
  | 150 | 100% | 100% |
  | 100（出力に白の余白 40px・2 倍） | **0%** | 100% |
  | 100（余白なし・600px 固定） | 100% | 100% |

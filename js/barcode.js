(() => {
  ...
  //
  // 0 のときは captureScanArea() が帯を塗らず、切り出した画像をそのまま渡す
  // （以前は一時的に 0 にしていた。そのころの戻し先は 50）
  const SCAN_PAD_X = 40;

  // SCAN_PAD_X の帯を何で塗るか（**試験的実装**）。
  //   'white'  白で塗る（従来）
  //   'edge'   切り出した画像の左端・右端の 1 列を、それぞれ帯の幅いっぱいに引き伸ばして塗る
  //            （行ごとに端の画素の色がそのまま続く）
  // 'edge' は、枠の端に台紙や背景の灰色が写っているときに、真っ白な帯との境目に
  // 段差（＝偽のバー）ができないようにする狙い。端がバーの途中で切れていると、
  // その黒が帯の幅いっぱいに伸びるので、クワイエットゾーンの代わりにはならない
  const SCAN_PAD_FILL = 'edge';
  ...
  function captureScanArea(rotate) {
    ...
    // 'edge' は映像を置いたあとに、その左端・右端の 1 列を帯の幅に引き伸ばして塗る。
    // 回転経路でも描いたあとの canvas の端を使うので、足す位置・向きは正立時と同じ。
    // 引き伸ばすのは横方向だけなので、行ごとの色はそのまま（補間で混ざらない）
    if (pad && SCAN_PAD_FILL === 'edge') {
      ctx.drawImage(canvas, pad, 0, 1, chh, 0, 0, pad, chh);
      ctx.drawImage(canvas, cw - pad - 1, 0, 1, chh, cw - pad, 0, pad, chh);
    }
